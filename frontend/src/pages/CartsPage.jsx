...
/**
 * CartsPage that uses cartUtils (which dispatches sasta:cart:changed)
 */

export default function CartsPage() {
  const [carts, setCarts] = useState(() => loadCarts());
  const providers = Object.keys(carts);
  const [active, setActive] = useState(providers[0] || 'instamart');
  ...
  function remove(provider, product_id, name) {
    const next = removeFromCart(provider, product_id);
    setCarts(next);
    window.dispatchEvent(new CustomEvent('sasta:toast', { detail: { message: `${name} removed from ${provider}`, actionText: null } }));
  }

  function empty(provider) {
    const next = clearCart(provider);
    setCarts(next);
    window.dispatchEvent(new CustomEvent('sasta:toast', { detail: { message: `${provider} cart cleared` } }));
  }
    ...
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        {providers.map(p => (
          ...
          }}>{p.charAt(0).toUpperCase() + p.slice(1)} ({(carts[p] || []).length})</button>
        ))}
      </div>
